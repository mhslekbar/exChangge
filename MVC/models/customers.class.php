...
class Customers extends Database {
    ...
    public function getTransCust($today,$brnch) {
        $stmt = $this->connect()->prepare("SELECT * FROM zztranscustomer tc JOIN zzcustomers c ON tc.tcIdCust = c.ccID WHERE substr(tcDate,'1','10') = ? AND tcIdBranch = ? ORDER BY tcID  DESC");
        $stmt->execute([$today,$brnch]);
